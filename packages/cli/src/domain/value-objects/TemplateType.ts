...
export class TemplateType {
    ...
    constructor(value: string) {
        if (!TemplateType.isValid(value)) {
            throw new Error(`Invalid template type: ${value}. Valid types are: ${TemplateType.VALID_TYPES.join(', ')}`);
        }
        this._value = value;
    }
    ...
    public static isValid(value: string): boolean {
        return TemplateType.VALID_TYPES.includes(value as typeof TemplateType.VALID_TYPES[number]);
    }
